...
import {
  Box, Heading, Text, Spinner, useColorModeValue,
  ...
  useEffect(() => {
    apiClient.get("/wallet/history")
      .then(res => {
        ...
        const processedData = Object.entries(monthlyStats).map(([monthKey, data]) => {
          const monthNumber = parseInt(monthKey.split('-')[1], 10);
          let income = 0;
          let expenditure = 0;

          Object.entries(data).forEach(([reason, amount]) => {
            if (['topup', 'bonus', 'freePoints', 'refund', 'adminCredit'].includes(reason)) {
              income += amount;
        ...
        }).sort((a, b) => monthNames.indexOf(a.name) - monthNames.indexOf(b.name)); // Sort by month
    ...
    <Box p={5}>
      ...
      {!loading && !error && Object.keys(stats).length > 0 && (
        <Box bg={cardBg} p={5} borderRadius="lg" shadow="md" mt={5}>
          <Heading size="lg" mb={4} color={textColor}>Monthly Summary</Heading>
      ...
      {!loading && !error && history.length > 0 && (
        <Box bg={cardBg} p={5} borderRadius="lg" shadow="md" mt={10}>
          <Heading size="lg" mb={4} color={textColor}>Transaction Details</Heading>
          <TableContainer>
                ...
                {history.map((item) => {
                  ...
                  return (
                    <Tr key={item._id}>
                      ...
                      <Td>{item.reason}</Td>
                      <Td isNumeric color={isPositive ? "green.500" : "red.500"}>
                        {isPositive ? '+' : ''}{amountChanged.toFixed(2)}
                      </Td>
      ...
      {!loading && !error && history.length === 0 && Object.keys(stats).length > 0 && (
        <Box bg={cardBg} p={5} borderRadius="lg" shadow="md" mt={10}>
            <Heading size="lg" mb={4} color={textColor}>Transaction Details</Heading>
      ...
      {!loading && !error && Object.keys(stats).length === 0 && history.length === 0 && (
         <Text mt={5} fontSize="lg">No wallet history or statistics available at the moment.</Text>
      )}
    </Box>
  );
};
